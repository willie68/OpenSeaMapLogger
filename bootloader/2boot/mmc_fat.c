...
static inline void check_file(direntry_t * dir) {
	...
	/*WKLA: 22.09.2013: Disabling the EEPROM Filename check. FIlename always be OSMFIRMW.HEX*/
#ifndef OSM	
...
#else
	if (dir->name[0] != 'O') return;
	if (dir->name[1] != 'S') return;
	if (dir->name[2] != 'M') return;
	if (dir->name[3] != 'F') return;
	if (dir->name[4] != 'I') return;
	if (dir->name[5] != 'R') return;
	if (dir->name[6] != 'M') return;
	if (dir->name[7] != 'W') return;
#endif
